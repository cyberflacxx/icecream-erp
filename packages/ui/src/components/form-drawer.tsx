...
export function FormDrawer({ title, open, onClose, children }: FormDrawerProps) {
  return (
    <Dialog.Root open={open} onOpenChange={(nextOpen) => (!nextOpen ? onClose() : undefined)}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-[60] bg-brown/40 backdrop-blur-sm dark:bg-darkBg/70" />
        <Dialog.Content className="fixed right-0 top-0 z-[70] flex h-full w-full max-w-xl flex-col bg-white shadow-2xl dark:bg-darkCard">
