...
exports.SUPPLIER_IMPORT_TEMPLATE_HEADERS = void 0;
exports.buildSupplierShortageRows = buildSupplierShortageRows;
exports.buildInvoiceAgeingRows = buildInvoiceAgeingRows;
exports.buildCostVarianceRows = buildCostVarianceRows;
exports.validateSupplierCodeUniqueness = validateSupplierCodeUniqueness;
exports.canPayInvoice = canPayInvoice;
exports.buildSupplierImportTemplateCsv = buildSupplierImportTemplateCsv;
exports.validateSupplierImportRows = validateSupplierImportRows;
exports.normalizeSupplierImportRow = normalizeSupplierImportRow;
exports.normalizeSupplierStatus = normalizeSupplierStatus;
exports.normalizePaymentTerms = normalizePaymentTerms;
exports.isValidSupplierPaymentTerms = isValidSupplierPaymentTerms;
exports.SUPPLIER_IMPORT_TEMPLATE_HEADERS = [
    'Supplier Code',
    'Supplier Name',
    'Contact Person',
    'Email Address',
...
function buildSupplierShortageRows(purchaseOrders) {
...
function buildInvoiceAgeingRows(invoices, paymentsByInvoiceId) {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    return invoices.map((invoice) => {
        ...
        const overdueDays = dueDate && balance > 0
...
function buildCostVarianceRows(invoices) {
    const rows = [];
...
function validateSupplierCodeUniqueness(existingCodes, nextCode) {
    return !existingCodes.map((code) => code.trim().toUpperCase()).includes(nextCode.trim().toUpperCase());
}
function canPayInvoice(balance, attemptedPayment) {
    return attemptedPayment > 0 && attemptedPayment <= balance;
}
function buildSupplierImportTemplateCsv() {
    const sampleRow = [
        'SUP-00001',
        'Example Supplier',
        'Patience Moyo',
    ...
    return [exports.SUPPLIER_IMPORT_TEMPLATE_HEADERS.join(','), sampleRow.join(',')].join('\n');
}
function validateSupplierImportRows(inputRows, existingCodes = []) {
...
function normalizeSupplierImportRow(row, rowNumber) {
...
function isValidSupplierPaymentTerms(value) {
