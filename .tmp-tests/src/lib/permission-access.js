...
exports.expandPermissionVariants = expandPermissionVariants;
...
function replaceTrailingAction(permission) {
    if (permission.endsWith('.view'))
        return permission.slice(0, -5) + '.read';
    if (permission.endsWith('.edit'))
        return permission.slice(0, -5) + '.update';
    return null;
}
function dotToUnderscoreVariant(permission) {
    const parts = permission.split('.');
    if (parts.length < 3)
        return null;
    return `${parts[0]}_${parts[1]}.${parts.slice(2).join('.')}`;
}
function expandPermissionVariants(permission) {
    ...
    if (underscored) {
        ...
        if (underscoredActionVariant)
            variants.add(underscoredActionVariant);
    }
    for (const alias of PERMISSION_ALIASES[normalized] ?? []) {
        variants.add(alias);
        const aliasActionVariant = replaceTrailingAction(alias);
        if (aliasActionVariant)
            variants.add(aliasActionVariant);
        const aliasUnderscored = dotToUnderscoreVariant(alias);
        if (aliasUnderscored)
            variants.add(aliasUnderscored);
    }
    return variants;
}
function hasPermissionAccess(grantedPermissions, ...requestedPermissions) {
