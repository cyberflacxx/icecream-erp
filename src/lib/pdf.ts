...
export interface BrandedPdfMetadataItem {
  label: string;
  value: string;
}

export interface BrandedPdfSummaryItem {
  label: string;
  value: string;
}

export interface BrandedPdfTableColumn {
...
export interface BrandedPdfSection {
  lines: string[];
  title: string;
}
...
export interface BrandedPdfDocumentOptions {
  ...
  generatedBy: string;
  logoPath?: string;
  metadata?: BrandedPdfMetadataItem[];
  sections?: BrandedPdfSection[];
  subtitle?: string;
  summary?: BrandedPdfSummaryItem[];
  table?: {
    columns: BrandedPdfTableColumn[];
...
const BRAND_NAVY: [number, number, number] = [24, 62, 124];
const BRAND_BLUE: [number, number, number] = [42, 97, 184];
const BRAND_SKY: [number, number, number] = [232, 241, 255];
const BRAND_BORDER: [number, number, number] = [191, 213, 245];
const TEXT_DARK: [number, number, number] = [23, 37, 84];
const TEXT_MUTED: [number, number, number] = [71, 85, 105];
const TEXT_LIGHT: [number, number, number] = [255, 255, 255];
const ROW_ALT: [number, number, number] = [248, 250, 252];
const EPSILON = 0.001;

type PdfObjectBody = Buffer | string;
...
function wrapTextToWidth(value: string, width: number, fontSize: number): string[] {
...
function fitText(value: string, width: number, fontSize: number) {
  ...
  if (!normalized) return '-';
  if (approxTextWidth(normalized, fontSize) <= width) return normalized;
...
function formatCellValue(value: unknown, keyHint = '') {
...
function getDefaultLogoPath() {
  const candidates = [
    path.join(process.cwd(), 'public', 'branding', 'logo.jpeg'),
    path.join(process.cwd(), 'public', 'branding', 'logo.jpg'),
...
function loadPdfLogo(logoPath?: string): PdfImageAsset | null {
...
function buildPdfDocument(input: {
  ...
  const setObject = (id: number, body: PdfObjectBody) => {
    objects[id] = body;
  };
  ...
  if (input.image && imageId) {
    const header = Buffer.from(
      `<< /Type /XObject /Subtype /Image /Width ${input.image.width} /Height ${input.image.height} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${input.image.buffer.length} >>\nstream\n`,
      'ascii',
    );
  ...
  for (const stream of input.pageStreams) {
    ...
    setObject(
      pageId,
      `<< /Type /Page /Parent ${pagesId} 0 R /MediaBox [0 0 ${PDF_PAGE_WIDTH} ${PDF_PAGE_HEIGHT}] /Resources << /Font << /F1 ${regularFontId} 0 R /F2 ${boldFontId} 0 R >> ${xObjectRef} >> /Contents ${contentId} 0 R >>`,
    );
...
export function createBrandedPdfDocument(options: BrandedPdfDocumentOptions): Uint8Array {
  const pages: string[][] = [];
  ...
  function drawHeader(page: string[]) {
    const bannerHeight = 96;
    const bannerY = PDF_PAGE_HEIGHT - bannerHeight;
    page.push(
      `${rgb(BRAND_NAVY)} rg`,
      `0 ${bannerY.toFixed(2)} ${PDF_PAGE_WIDTH} ${bannerHeight} re f`,
      `${rgb(BRAND_BLUE)} rg`,
      `0 ${(bannerY - 8).toFixed(2)} ${PDF_PAGE_WIDTH} 8 re f`,
    );
    ...
    page.push(
      ...
      `/F2 13 Tf`,
      `${rgb(TEXT_LIGHT)} rg`,
      `1 0 0 1 ${titleX.toFixed(2)} ${titleY.toFixed(2)} Tm`,
      `(Absolute Ice Cream ERP) Tj`,
      'ET',
      'BT',
      `/F2 22 Tf`,
      `${rgb(TEXT_LIGHT)} rg`,
      `1 0 0 1 ${titleX.toFixed(2)} ${subtitleY.toFixed(2)} Tm`,
    ...
    if (options.subtitle) {
      page.push(
        ...
        `/F1 10 Tf`,
        `${rgb(TEXT_LIGHT)} rg`,
        `1 0 0 1 ${titleX.toFixed(2)} ${(subtitleY - 18).toFixed(2)} Tm`,
        `(${escapePdfText(fitText(options.subtitle, 230, 10))}) Tj`,
        'ET',
      );
    }
    ...
    for (const item of headerMeta.slice(0, 6)) {
      ...
      for (const wrapped of lines) {
        page.push(
          ...
          `/F1 9 Tf`,
          `${rgb(TEXT_LIGHT)} rg`,
          `1 0 0 1 ${(PDF_PAGE_WIDTH - PAGE_MARGIN - 184).toFixed(2)} ${(PDF_PAGE_HEIGHT - metaTop - 10).toFixed(2)} Tm`,
          `(${escapePdfText(fitText(wrapped, 184, 9))}) Tj`,
          'ET',
        );
        metaTop += 12;
      }
  ...
  function drawFooter(page: string[], pageNumber: number, pageCount: number) {
    const footerTop = PDF_PAGE_HEIGHT - FOOTER_HEIGHT;
    page.push(
      `${rgb(BRAND_BORDER)} RG`,
      `${PAGE_MARGIN} ${footerTop.toFixed(2)} m ${PDF_PAGE_WIDTH - PAGE_MARGIN} ${footerTop.toFixed(2)} l S`,
    );
    ...
    page.push(
      ...
      `/F2 8 Tf`,
      `${rgb(TEXT_DARK)} rg`,
      `1 0 0 1 ${PAGE_MARGIN.toFixed(2)} 24 Tm`,
      `(${escapePdfText(leftText)}) Tj`,
      'ET',
      'BT',
      `/F1 8 Tf`,
      `${rgb(TEXT_MUTED)} rg`,
      `1 0 0 1 ${(PAGE_MARGIN + 120).toFixed(2)} 24 Tm`,
      `(${escapePdfText(centerText)}) Tj`,
      'ET',
      'BT',
      `/F1 8 Tf`,
      `${rgb(TEXT_MUTED)} rg`,
      `1 0 0 1 ${(PAGE_MARGIN + 250).toFixed(2)} 24 Tm`,
      `(${escapePdfText(fitText(footerText, 150, 8))}) Tj`,
      'ET',
      'BT',
      `/F1 8 Tf`,
      `${rgb(TEXT_MUTED)} rg`,
      `1 0 0 1 ${(PDF_PAGE_WIDTH - PAGE_MARGIN - 150).toFixed(2)} 24 Tm`,
      `(${escapePdfText(fitText(rightText, 150, 8))}) Tj`,
      'ET',
    );
  }

  function drawSummarySection(items: BrandedPdfSummaryItem[]) {
    if (!items.length) {
      return;
    }
    ...
    for (let index = 0; index < items.length; index += cardsPerRow) {
      ...
      rowItems.forEach((item, rowIndex) => {
        ...
        currentPage.push(
          `${rgb(BRAND_SKY)} rg`,
          `${rgb(BRAND_BORDER)} RG`,
          `${x.toFixed(2)} ${y.toFixed(2)} ${cardWidth.toFixed(2)} ${cardHeight.toFixed(2)} re B`,
          'BT',
          `/F2 10 Tf`,
          `${rgb(BRAND_BLUE)} rg`,
          `1 0 0 1 ${(x + 14).toFixed(2)} ${(y + cardHeight - 18).toFixed(2)} Tm`,
          `(${escapePdfText(fitText(item.label, cardWidth - 28, 10))}) Tj`,
          'ET',
          'BT',
          `/F2 16 Tf`,
          `${rgb(TEXT_DARK)} rg`,
          `1 0 0 1 ${(x + 14).toFixed(2)} ${(y + 18).toFixed(2)} Tm`,
          `(${escapePdfText(fitText(item.value, cardWidth - 28, 16))}) Tj`,
          'ET',
        );
      });
  ...
  function drawSectionHeading(title: string) {
    ...
    currentPage.push(
      ...
      `/F2 12 Tf`,
      `${rgb(TEXT_DARK)} rg`,
      `1 0 0 1 ${PAGE_MARGIN.toFixed(2)} ${(PDF_PAGE_HEIGHT - cursorTop - 12).toFixed(2)} Tm`,
  ...
  function drawSections(sections: BrandedPdfSection[]) {
      ...
      for (const line of section.lines) {
        ...
        for (const wrappedLine of wrapped) {
          currentPage.push(
            ...
            `/F1 10 Tf`,
            `${rgb(TEXT_MUTED)} rg`,
            `1 0 0 1 ${PAGE_MARGIN.toFixed(2)} ${(PDF_PAGE_HEIGHT - cursorTop - 10).toFixed(2)} Tm`,
  ...
  function resolveColumnWidths(columns: BrandedPdfTableColumn[]) {
  ...
  function drawTableHeader(columns: Array<BrandedPdfTableColumn & { width: number }>, title?: string) {
    if (title) {
    ...
    currentPage.push(
      `${rgb(BRAND_NAVY)} rg`,
      `${rgb(BRAND_NAVY)} RG`,
      `${PAGE_MARGIN.toFixed(2)} ${y.toFixed(2)} ${(PDF_PAGE_WIDTH - PAGE_MARGIN * 2).toFixed(2)} ${rowHeight.toFixed(2)} re B`,
    );
    ...
    for (const column of columns) {
      ...
      currentPage.push(
        ...
        `/F2 9 Tf`,
        `${rgb(TEXT_LIGHT)} rg`,
        `1 0 0 1 ${textX.toFixed(2)} ${(y + 8).toFixed(2)} Tm`,
  ...
  function drawTable(table: NonNullable<BrandedPdfDocumentOptions['table']>) {
    ...
    if (!table.rows.length) {
      ...
      currentPage.push(
        `${rgb(BRAND_SKY)} rg`,
        `${rgb(BRAND_BORDER)} RG`,
        `${PAGE_MARGIN.toFixed(2)} ${y.toFixed(2)} ${(PDF_PAGE_WIDTH - PAGE_MARGIN * 2).toFixed(2)} 32 re B`,
        'BT',
        `/F1 10 Tf`,
        `${rgb(TEXT_MUTED)} rg`,
        `1 0 0 1 ${(PAGE_MARGIN + 12).toFixed(2)} ${(y + 11).toFixed(2)} Tm`,
        '(No rows returned for the selected filters.) Tj',
        'ET',
      );
      cursorTop += 40;
      return;
    ...
    table.rows.forEach((row, rowIndex) => {
      ...
      const rowLineCounts = cellValues.map((value, index) =>
        wrapTextToWidth(value, columns[index]?.width ? columns[index].width - 12 : 80, 9).length,
      );
      ...
      const y = PDF_PAGE_HEIGHT - cursorTop - rowHeight;
      currentPage.push(
        `${rgb(rowIndex % 2 === 0 ? [255, 255, 255] : ROW_ALT)} rg`,
        `${rgb(BRAND_BORDER)} RG`,
        `${PAGE_MARGIN.toFixed(2)} ${y.toFixed(2)} ${(PDF_PAGE_WIDTH - PAGE_MARGIN * 2).toFixed(2)} ${rowHeight.toFixed(2)} re B`,
      );
      ...
      columns.forEach((column, columnIndex) => {
        ...
        wrapped.forEach((line, lineIndex) => {
          ...
          currentPage.push(
            ...
            `/F1 9 Tf`,
            `${rgb(TEXT_DARK)} rg`,
            `1 0 0 1 ${textX.toFixed(2)} ${(y + rowHeight - 14 - lineIndex * 11).toFixed(2)} Tm`,
