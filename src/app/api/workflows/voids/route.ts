...
import { createVoidRequest, listVoids } from '@/lib/workflow-server';
...
export async function GET(request: NextRequest) {
  ...
  try {
    return workflowResponse(await listVoids(auth.ctx.organizationId));
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  if ('error' in auth) return auth.error;
  try {
    return workflowResponse(await createVoidRequest({ body: await request.json(), ctx: auth.ctx }), 201);
  } catch (error) {
    return workflowError(error);
  }
}
