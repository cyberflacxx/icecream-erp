...
export async function GET(request: NextRequest) {
  ...
  try {
    return adminResponse(await listOpeningBalances('customers', auth.ctx.organizationId));
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  if ('error' in auth) return auth.error;
  try {
    return adminResponse(await createOpeningPartyBalance('customers', await request.json(), auth.ctx), 201);
  } catch (error) {
    return adminError(error);
  }
}
