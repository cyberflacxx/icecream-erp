...
import { summarizeBalanceSheetFromLedger, summarizeProfitAndLossFromLedger } from '@/lib/finance';
...
export async function GET() {
  ...
  try {
    const [ledgerLines, invoices, branchSales, expenses, branchExpenses] = await Promise.all([
    ...
    const balance = summarizeBalanceSheetFromLedger(ledgerLines);
    const ledgerPnl = summarizeProfitAndLossFromLedger(ledgerLines);
    ...
    const fallbackOperatingExpenses =
      expenses
    ...
    const revenue = ledgerPnl.revenue || fallbackRevenue;
    const operatingExpenses = ledgerPnl.operatingExpenses || fallbackOperatingExpenses;
    const netProfit = ledgerPnl.netProfit || revenue - operatingExpenses;
