...
export async function GET(request: NextRequest) {
  ...
  try {
    return NextResponse.json(await listItems('PACKAGING_MATERIAL', auth.ctx.organizationId));
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    return await createItemResponse({
      body: { ...((await request.json()) as Record<string, unknown>), itemType: 'PACKAGING_MATERIAL' } as {
        categoryId: string;
        code: string;
        description?: string;
        isActive?: boolean;
