...
import { createDocumentation, listDocumentation } from '@/lib/testing-server';
...
export async function GET(request: NextRequest) {
  ...
  try {
    return testingResponse(await listDocumentation(auth.ctx));
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  if ('error' in auth) return auth.error;
  try {
    return testingResponse(await createDocumentation(auth.ctx, await request.json()), 201);
  } catch (error) {
    return testingError(error);
  }
}
