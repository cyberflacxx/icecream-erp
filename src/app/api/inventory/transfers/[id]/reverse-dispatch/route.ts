...
import { mapInventoryReversalError, reverseStockTransferDispatch } from '@/lib/inventory-reversal-server';
...
export async function POST(
  ...
  if (!can(ctx, 'inventory.transfer.reverse_dispatch', 'inventory.write')) return forbidden();
  ...
  try {
    ...
    const branchId = await loadWarehouseBranchId(service as never, fromWarehouseId);
    ...
    const result = await reverseStockTransferDispatch({
      ...
      idempotencyKey: buildInventoryPostingIdempotencyKey({
        actorUserId: ctx.userId,
        documentId: id,
        operation: 'stock_transfer_dispatch_reverse',
      }),
