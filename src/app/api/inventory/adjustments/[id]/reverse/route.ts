...
import { mapInventoryReversalError, reverseInventoryAdjustment } from '@/lib/inventory-reversal-server';
...
export async function POST(
  ...
  if (!can(ctx, 'inventory.adjustment.reverse', 'inventory.write')) return forbidden();

  try {
    const { id } = await params;
    const service = createServiceRoleClient();
    const { data: adjustment, error } = await service
    ...
    const warehouseId = String(adjustment.warehouse_id ?? '').trim();
    if (!warehouseId) {
      return badRequest('Stock adjustment is missing its warehouse.');
    }
    ...
    const result = await reverseInventoryAdjustment({
      ...
      idempotencyKey: buildInventoryPostingIdempotencyKey({
        actorUserId: ctx.userId,
        documentId: id,
        operation: 'stock_adjustment_reverse',
      }),
