...
import { mapInventoryReversalError, reverseInventoryWriteOff } from '@/lib/inventory-reversal-server';
...
export async function POST(
  ...
  if (!can(ctx, 'inventory.writeoff.reverse', 'inventory.write_off.reverse', 'inventory.write')) return forbidden();
  ...
  try {
    ...
    if (!batch) return notFound('Inventory write-off batch not found.');

    const warehouseId = String(batch.warehouse_id ?? '').trim();
    if (!warehouseId) {
      return badRequest('Inventory batch is missing its warehouse.');
    }
    ...
    const result = await reverseInventoryWriteOff({
      ...
      idempotencyKey: buildInventoryPostingIdempotencyKey({
        actorUserId: ctx.userId,
        documentId: id,
        operation: 'inventory_write_off_reverse',
      }),
