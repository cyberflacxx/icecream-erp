...
const ROLES_UNCONFIGURED_MESSAGE = 'Registration roles are not configured. Please contact the administrator.';
...
function normalizeRoleOptions(payload: { data?: RoleOption[] } | RoleOption[]) {
...
export default function RegisterPageClient({
  ...
  const [isRolesLoading, setIsRolesLoading] = useState(initialRoles.length === 0);
  const [isRolesRefreshing, setIsRolesRefreshing] = useState(false);
  ...
  const rolesRef = useRef<RoleOption[]>(initialRoles);
  ...
  async function loadRoles(signal?: AbortSignal) {
    const hasExistingRoles = rolesRef.current.length > 0;

    setRolesError(null);
    setIsRolesLoading(!hasExistingRoles);
    setIsRolesRefreshing(hasExistingRoles);

    try {
      const response = await fetch(`/api/roles/public?ts=${Date.now()}`, {
        cache: 'no-store',
        headers: {
      ...
      const payload = (await response.json().catch(() => ({}))) as { data?: RoleOption[]; error?: string } | RoleOption[];
      const roleArray = normalizeRoleOptions(payload);
        ...
        if (!hasExistingRoles) {
          ...
          setRolesError(
            Array.isArray(payload)
              ? ROLES_LOAD_FAILURE_MESSAGE
              : payload.error ?? ROLES_LOAD_FAILURE_MESSAGE,
          );
        }
        return;
      }
      ...
      if (roleArray.length === 0) {
        if (!hasExistingRoles) {
          setRoles([]);
          setRolesError(ROLES_UNCONFIGURED_MESSAGE);
        }
        return;
      }
  ...
  useEffect(() => {
    if (roleId && !roles.some((role) => role.id === roleId)) {
      setRoleId('');
      setFieldErrors((current) => ({ ...current, role: 'Please select a role.' }));
    }
  }, [roleId, roles]);
    ...
    >
      <div className="auth-card">
        ...
        <p className="mt-2 text-sm text-muted dark:text-darkMuted">Enter your details, request an OTP, then verify it to create the account.</p>
          ...
          {isRolesRefreshing && roles.length > 0 ? (
            <p className="mt-4 text-xs text-muted">Refreshing available roles...</p>
          ) : null}
          {otpRequestId ? (
          ...
          <form className="mt-6 space-y-4" onSubmit={(event) => event.preventDefault()}>
            ...
            <label className="block space-y-2">
              ...
              <select
                value={roleId}
                disabled={(isRolesLoading && roles.length === 0) || registrationLocked}
                onChange={(event) => {
              ...
              >
                <option value="">{isRolesLoading && roles.length === 0 ? 'Loading roles...' : rolesError ? 'Roles unavailable' : 'Select role'}</option>
                {roles.map((role) => (
            ...
            <div className="space-y-3">
              {!otpRequestId ? (
                <button
                  type="button"
                  disabled={!canRequestOtp || (roles.length === 0 && Boolean(rolesError))}
                  onClick={requestOtp}
